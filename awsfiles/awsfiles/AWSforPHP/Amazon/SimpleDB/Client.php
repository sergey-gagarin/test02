...
class Amazon_SimpleDB_Client implements Amazon_SimpleDB_Interface
{
     ...
     * 
     * @see http://docs.amazonwebservices.com/AmazonSimpleDB/2007-11-07/DeveloperGuide/SDB_API_DomainMetadata.html
     ...
     * @param mixed $request array of parameters for Amazon_SimpleDB_Model_QueryRequest request
     * or Amazon_SimpleDB_Model_QueryRequest object itself
    ...
    {
        if (!$request instanceof Amazon_SimpleDB_Model_QueryRequest) {
            require_once ('Amazon/SimpleDB/Model/QueryRequest.php');
            $request = new Amazon_SimpleDB_Model_QueryRequest($request);
        }
        require_once ('Amazon/SimpleDB/Model/QueryResponse.php');
        return Amazon_SimpleDB_Model_QueryResponse::fromXML($this->_invoke($this->_convertQuery($request)));
    }
     ...
     * 
     * @see http://docs.amazonwebservices.com/AmazonSimpleDB/2007-11-07/DeveloperGuide/SDB_API_Select.html
     ...
     * @param mixed $request array of parameters for Amazon_SimpleDB_Model_QueryWithAttributesRequest request
     * or Amazon_SimpleDB_Model_QueryWithAttributesRequest object itself
     * @see Amazon_SimpleDB_Model_QueryWithAttributes
     * @return Amazon_SimpleDB_Model_QueryWithAttributesResponse Amazon_SimpleDB_Model_QueryWithAttributesResponse
     *
     * @throws Amazon_SimpleDB_Exception
     */
    public function queryWithAttributes($request)
    {
        if (!$request instanceof Amazon_SimpleDB_Model_QueryWithAttributesRequest) {
            require_once ('Amazon/SimpleDB/Model/QueryWithAttributesRequest.php');
            $request = new Amazon_SimpleDB_Model_QueryWithAttributesRequest($request);
        }
        require_once ('Amazon/SimpleDB/Model/QueryWithAttributesResponse.php');
        return Amazon_SimpleDB_Model_QueryWithAttributesResponse::fromXML($this->_invoke($this->_convertQueryWithAttributes($request)));
    }
    ...
    private function _convertBatchPutAttributes($request) {
        ...
        foreach ($request->getItem() as $itemIndex => $item) {
            if ($item->isSetItemName()) {
                $parameters['Item' . '.'  . ($itemIndex + 1) . '.' . 'ItemName'] =  $item->getItemName();
            }
            foreach ($item->getAttribute() as $attributeIndex => $attribute) {
                if ($attribute->isSetName()) {
                    $parameters['Item' . '.'  . ($itemIndex + 1) . '.' . 'Attribute' . '.'  . ($attributeIndex + 1) . '.' . 'Name'] =  $attribute->getName();
                }
                if ($attribute->isSetValue()) {
                    $parameters['Item' . '.'  . ($itemIndex + 1) . '.' . 'Attribute' . '.'  . ($attributeIndex + 1) . '.' . 'Value'] =  $attribute->getValue();
                }
                if ($attribute->isSetReplace()) {
                    $parameters['Item' . '.'  . ($itemIndex + 1) . '.' . 'Attribute' . '.'  . ($attributeIndex + 1) . '.' . 'Replace'] =  $attribute->getReplace() ? "true" : "false";
                }
    ...
    /**
     * Convert QueryRequest to name value pairs
     */
    private function _convertQuery($request) {
        ...
        if ($request->isSetQueryExpression()) {
            $parameters['QueryExpression'] =  $request->getQueryExpression();
        }
        if ($request->isSetMaxNumberOfItems()) {
            $parameters['MaxNumberOfItems'] =  $request->getMaxNumberOfItems();
        }
        if ($request->isSetNextToken()) {
            $parameters['NextToken'] =  $request->getNextToken();
        }
    ...
    /**
     * Convert QueryWithAttributesRequest to name value pairs
     */
    private function _convertQueryWithAttributes($request) {
        
        $parameters = array();
        $parameters['Action'] = 'QueryWithAttributes';
        foreach  ($request->getAttributeName() as $attributeNameIndex => $attributeName) {
            $parameters['AttributeName' . '.'  . ($attributeNameIndex + 1)] =  $attributeName;
        }
        if ($request->isSetDomainName()) {

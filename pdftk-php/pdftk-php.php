...
#
#	Usage: 
#		$pdfmaker = new pdftk_php("<path to pdftk>");
#
...
/**
 * To have the script working on local and Flow production server
 * put the pdftk.exe and libiconv2.ddl in C:\pdftk\bin\
 * **/
	class pdftk_php { 		

		var $path;
		
		function pdftk_php($path=null) {
			if (!$path) {
				$path = array('/usr/bin/pdftk',"C:\\pdftk\\bin\\pdftk.exe");
			}
			if (is_scalar($path)) {
				$this->path = $path;
		 ...
		 * 
		 * @param $originals array() of files to be merged
		 * @param $outputfile the output file, will be sent to browser if null!
		 */
		function merge_pdf($originals, $outputfile=null) {
			if(!(count($originals)>0)) return null;
			...
			if (!$outputfile) {
				$tmp = tempnam(FILE_ROOT."/temp", "pdf");
				
				// temp result file is required for catenation the output can't be pushed to the browser on the flow: 
				$cmd2 = $this->path." $filesToCat cat output $tmp";
				passthru($cmd2);
				...
				$cmd2 = $this->path." $filesToCat cat output $outputfile";
				passthru($cmd2);
			}
		} 
/**
 * 
 * make sure that temporary dir - $resDir = FILE_ROOT."/temp/"; exists. 
 * 
 * **/
	function createFilledForm(		$fdf_data_strings,  // data to fill in
									$fdf_data_names,    
									$fields_hidden, 
									$fields_readonly, 
									$pdf_originals, 
									...
									$createFlatten=false) 
		{
			...
			if(!(count($pdf_originals)>0)) return null;
			...
			if($fp) {
				...
				// To make the PDF by running pdftk - make sure the path to pdftk is set correctly in constructor.
								
				$pdfFilledFiles = array();
				$filesToCat = '';
				
				foreach ($pdf_originals as $f){
										
					// create temp files in default sys dir:
					$pdfTempFilled = tempnam(".", "pdf");
					
					if($createFlatten)
					{
						$cmd1 = $this->path." $f fill_form $fdf_fn output $pdfTempFilled flatten";
					}else{
						$cmd1 = $this->path." $f fill_form $fdf_fn output $pdfTempFilled";
					}
					
					// excute pdftk.exe to fill the data in temp files:
					passthru($cmd1);
					
					rename($pdfTempFilled, $pdfTempFilled.".pdf");
					
					$pdfFilledFiles[] = $pdfTempFilled;
					
					// all the files as a string:
					$filesToCat .= " ".$pdfTempFilled.".pdf";
					
					$pdfTempFilled = null;
				}
				
				// catenated files should be placed somewhere:
				$resDir = FILE_ROOT."/temp/";
				// $resDir = FILE_ROOT."/forms/operations/temp/";
				
				// result file:
				$res = $resDir.$pdf_result;
				
				// temp result file is required for catenation: 
				$cmd2 = $this->path." $filesToCat cat output $res";
				
				passthru($cmd2);
				
				// delete temporary files:
				foreach ($pdfFilledFiles  as $p)
				{
					unlink( $p.".pdf" );
				}
				unlink( $fdf_fn ); 
				...
				// Send a force download header to the browser with a file MIME type and file name passed to the function:
				header("Content-Type: application/force-download");
				header("Content-Disposition: attachment; filename=\"$pdf_result\"");
...
#############################################################################		
		public function make_pdf($fdf_data_strings, $fdf_data_names, $fields_hidden, $fields_readonly, $pdf_original, $pdf_filename) {
			...
			if($fp) {
				...
				passthru($this->path." $pdf_original fill_form $fdf_fn output - flatten");
